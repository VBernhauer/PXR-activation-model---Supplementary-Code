function [] = batch_mcmc(num)

    %%% command: batch_mcmc(num), where num = 1 or 2 or 3 or 4 or 5
    %%% note: adjust nprocs parameter (line 21) to the number of cores one
    %%% wants to use
    ...
    if ~exist(strcat('./chains_',num2str(num)),'dir')
        mkdir(strcat('./chains_',num2str(num)))
    end

    %%% load initial parameters
    parameters = load(strcat('paramsample_',num2str(num),'.mat'));
    ...
    for iproc = 1:nprocs
        ...
        task{iproc} = createTask(j, @mcmc, 1, {nstart(iproc),nend(iproc),parameters,num});
        fprintf('done!\n');
    end
